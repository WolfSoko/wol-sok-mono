...
export class PerformanceTestService {
  ...
  startTest(performanceTest: PerformanceTest): Observable<PerformanceTestResults> {
  ...
  private createArrayTest(): Observable<PerformanceTestResults> {
    return defer(() => {
      ...
      testArray.reduce((previousValue, currentValue) => previousValue + currentValue);
      ...
      const arrayCreation = PerformanceTestService.calcMeanTime('arrayCreation');
      const completeTime = PerformanceTestService.calcMeanTime('arrayReduceTest');
  ...
  private createRxTest(): Observable<PerformanceTestResults> {
    ...
    return range(0, ARRAY_ITEMS_LENGTH, queueScheduler).pipe(
      ...
      map(() => {
        const completeTime = PerformanceTestService.calcMeanTime('rxReduceTest');
