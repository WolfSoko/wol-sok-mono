...
describe('ShowFpsComponent', () => {
  ...
  it('should show fps if show is true', () => {
    const spectator = createHost(`<ws-shared-ui-show-fps [show]="true" [fps]="40.1" ></ws-shared-ui-show-fps>`);
    expect(spectator.query(byText('40.1 FPS'))).toExist();
  });

  it('should not show fps if show is false', () => {
    const spectator = createHost(`<ws-shared-ui-show-fps [show]="false" [fps]="40.1" ></ws-shared-ui-show-fps>`);
    expect(spectator.query(byText('40.1 FPS'))).not.toExist();
  });

  it('should update fps when changes', () => {
    const spectator: SpectatorHost<ShowFpsComponent, { fps: number }> = createHost(
  ...
  it('should hide when show changes', () => {
    const spectator: SpectatorHost<ShowFpsComponent, { showFps: boolean }> = createHost(
