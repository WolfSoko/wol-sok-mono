...
export class ReactionDiffWorkerCalcService implements ReactionDiffCalculator {
  ...
  addChemical(x: number, y: number): void {
    ...
    const workerParams: WorkerPostParams<AddChemicalsParams> = {
      data: data,
      transferList: [gridCopy.buffer],
    };
