...
export class WeightsConfigComponent {
  ...
  @Output() weightsChanged: Observable<CellWeights> = this.weightsChangedSubject.pipe(
  ...
  private sumWeightsAndCheckValidity(weights: CellWeights = this.weights): SumOfWeights {
    if (weights) {
